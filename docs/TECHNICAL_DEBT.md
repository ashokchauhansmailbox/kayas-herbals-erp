...
|---|---|---|---|---|---|
| TD-11 | **~90 missing FastAPI route return-type annotations** surface under `mypy --strict`. The project's `mypy.ini` runs in a slightly relaxed mode (untyped `def` allowed on handler bodies where the return type is inferred from `response_model`). Tightening to `--strict` across the codebase is a housekeeping pass. | Non-functional; project mypy config passes with 0 issues. `--strict` gate is aspirational. | Low | 1.6 | Open |
| TD-09 | **PO / GRN / vendor-invoice transactional workflows** are schema-only in Sprint 1.4. Endpoints for PO approval, GRN posting into `stock_ledger`, and vendor-invoice reconciliation land in Sprint 1.5 alongside Orders + Billing. | Composes naturally with sale-side workflows in Sprint 1.5. | Medium | 1.5 | Open |
| TD-10 | **19 new FK columns on Sprint-1.4 tables lack B-tree indexes** (mostly `_by` actor FKs on `vendors`, `purchase_orders`, `grn`, `vendor_invoices`, `distributor_tiers`, `distributors`, `price_lists`, `kyc_documents`, `customer_profiles`, `addresses`, `wallets`, `referrals`). | Batched with TD-01 for a single Sprint 1.6 index migration. | Medium | 1.6 | Open |
